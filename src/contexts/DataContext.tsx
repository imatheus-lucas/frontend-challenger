...
export const DataContextProvider = ({ children }: any) => {
  const [selectedPatient, setSelectedPatient] = useState<listPatientsProps | undefined>();
  ...
  const fetchPatients = async () => {
    try {
      setLoadMore(true);
      const { data } = await api.get(`/?page=${page}&results=50`);
      if (data.results) {
        const patients = data.results.map((element: any) => {
          ...
          return {
            ...
            shareUrl: `http://localhost:3000/${element.login.uuid}`,
          };
        });
  ...
  return (
    <DataContext.Provider value={{ listPatients, nextPage, loading, loadMore,selectPatient,selectedPatient }}>
      {children}
